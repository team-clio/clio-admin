...
export function useCreateBug(projectId: number | null) {
  return useMutation({
    mutationFn: (payload: CreateBugRequest) =>
      createBug(projectId as number, payload),
  });
}
...
/** 이슈 최신 AI 분석 결과 */
export function useLatestIssueAnalysis(
  projectId: number | null,
  issueId: number | null,
) {
  return useQuery({
    ...
    queryFn: () =>
      getLatestIssueAnalysis(projectId as number, issueId as number),
    enabled: projectId !== null && issueId !== null,
  });
}

/** 이슈 코드 근거 파일 */
export function useCodeEvidence(
  projectId: number | null,
  issueId: number | null,
) {
  return useQuery({
...
export function useProjectDocuments(projectId: number | null) {
  return useQuery({
    ...
    queryFn: () => getProjectDocuments(projectId as number),
    enabled: projectId !== null,
  });
}

/** 프로젝트 참고 문서 업로드 */
export function useCreateProjectDocument(projectId: number | null) {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({ title, file }: { title: string; file: File }) =>
      createProjectDocument(projectId as number, title, file),
    onSuccess: () =>
      queryClient.invalidateQueries({
        queryKey: ["project-documents", projectId],
      }),
...
/** 프로젝트 참고 문서 삭제 */
export function useDeleteProjectDocument(projectId: number | null) {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (documentId: number) =>
      deleteProjectDocument(projectId as number, documentId),
    onSuccess: () =>
      queryClient.invalidateQueries({
        queryKey: ["project-documents", projectId],
      }),
...
export function usePcmKnowledgeDetail(
  ...
  return useQuery({
    ...
    queryFn: () =>
      getPcmKnowledgeDetail(projectId as number, knowledgeId as string),
    enabled: projectId !== null && knowledgeId !== null,
  });
}

export type { ProjectDocument, ProjectRepository };
