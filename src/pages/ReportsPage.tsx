...
import { AlertCircle, Bug, Clock3, FileText, Link2, MoreHorizontal, Server } from 'lucide-react'
import { getBugs, type BugSummary, type BugStatus, type Severity } from '../api/bugs'
import { Button, IconButton, PageHeader, Surface } from '../components/ui'
...
const statusStyle: Record<BugStatus, string> = {
  NEW: 'bg-amber-50 text-amber-700', ANALYZING: 'bg-blue-50 text-blue-700', TRIAGED: 'bg-emerald-50 text-emerald-700', RESOLVED: 'bg-slate-100 text-slate-600', IGNORED: 'bg-slate-100 text-slate-400',
}
const severityLabel: Record<Severity, string> = { CRITICAL: '긴급', HIGH: '높음', MEDIUM: '보통', LOW: '낮음' }
const severityStyle: Record<Severity, string> = { CRITICAL: 'text-rose-600', HIGH: 'text-orange-600', MEDIUM: 'text-amber-600', LOW: 'text-slate-400' }
