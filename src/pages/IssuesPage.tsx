  ...
  if (projectId === null) {
    return (
      <div className="animate-page flex min-h-[calc(100vh-3.5rem)] flex-col">
        <PageHeader eyebrow="PRIORITIZED QUEUE" title="이슈" description="서버에서 계산된 우선순위와 연결 버그를 확인합니다." />
        <div className="flex-1"><NoProjectSelected /></div>
      </div>
    )
  }
      ...
      <div className="flex-1">
        {error && (
          <div className="px-4 pt-4 lg:px-8">
            <p role="alert" className="flex items-center gap-2 rounded-lg bg-rose-50 p-3 text-xs font-bold text-rose-700">
              <AlertCircle size={15} />
              {error}
            </p>
          </div>
        ...
        <div className="flex items-center justify-between border-b border-slate-200 bg-white px-5 py-4 lg:px-8">
          <div><p className="text-sm font-bold text-slate-800">작업 큐</p><p className="mt-0.5 text-xs text-slate-400">{loading ? '불러오는 중...' : `열린 이슈 ${stats?.openIssues ?? 0} · 전체 ${stats?.totalIssues ?? items.length}`}</p></div>
          <span className="rounded-full bg-clio-50 px-2.5 py-1 text-xs font-bold text-clio-700">{filteredItems.length}</span>
        </div>
        <div className="flex flex-col gap-3 border-b border-slate-200 bg-white px-5 py-3 sm:flex-row sm:items-center sm:justify-between lg:px-8">
          <label className="flex min-w-0 flex-1 items-center gap-2 rounded-lg border border-slate-200 px-3 py-2 text-slate-400 transition-all duration-200 focus-within:border-clio-500 focus-within:ring-2 focus-within:ring-blue-100 sm:max-w-xs">
            <Search size={15} />
            <input value={query} onChange={(event) => setQuery(event.target.value)} className="min-w-0 flex-1 bg-transparent text-xs text-slate-700 outline-none placeholder:text-slate-400" placeholder="제목, 요약, ID로 검색" />
          </label>
          <div className="flex items-center gap-2 overflow-x-auto">
            <div className="flex gap-1.5">
              {statusFilters.map((filter) => <button key={filter.id} onClick={() => setStatusFilter(filter.id)} className={`shrink-0 rounded-full px-3 py-1.5 text-xs font-bold transition-colors ${statusFilter === filter.id ? 'bg-clio-600 text-white' : 'bg-slate-100 text-slate-500 hover:bg-slate-200'}`}>{filter.label}</button>)}
            </div>
        ...
        {!loading && items.length === 0 ? (
          <div className="grid h-80 place-items-center bg-white text-sm text-slate-400">
            {projectId ? '등록된 이슈가 없습니다.' : '프로젝트를 먼저 선택하세요.'}
          </div>
            ...
            {filteredItems.map((issue) => (
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  <h3 className="line-clamp-2 text-sm font-bold leading-5 text-slate-800">{issue.title}</h3>
                  <div className="mt-1.5 flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-slate-400">
                    <span className="font-mono">ISS-{issue.id}</span>
                    <span className={`rounded-full px-2 py-0.5 text-[11px] font-bold ${statusStyle[issue.status]}`}>{statusLabel[issue.status]}</span>
                    <span className="flex items-center gap-1"><FileText size={12} /> {issue.bugCount}</span>
...
function IssueDetailPanel({ selected, detailLoading, analysis, codeEvidence, analysisLoading, activeTab, setActiveTab, updating, changeStatus, onClose }: {
  ...
  return <div className="animate-detail min-h-full">
    ...
    <div id={`issue-panel-${activeTab}`} role="tabpanel" aria-labelledby={`issue-tab-${activeTab}`} className={activeTab === 'evidence' ? 'min-h-[calc(100vh-10.5rem)] p-0' : 'mx-auto max-w-4xl p-5 lg:p-8'}>
...
function CodeEvidenceIde({ data, fallback }: { data: CodeEvidenceResponse | null; fallback: NonNullable<IssueAnalysisSnapshot['evidence']> }) {
  ...
  return <div className="grid min-h-[calc(100vh-10.5rem)] grid-cols-[17rem_minmax(0,1fr)] bg-slate-950"><aside className="border-r border-slate-700 bg-slate-900 p-4"><p className="mb-4 text-[10px] font-bold tracking-widest text-slate-400">EVIDENCE FILES</p>{files.map((item, index) => <button key={`${item.repository_id}-${item.path}`} onClick={() => setSelected(index)} className={`mb-1 block w-full truncate rounded px-2.5 py-2 text-left font-mono text-xs ${index === selected ? 'bg-clio-600 text-white' : 'text-slate-300 hover:bg-slate-800'}`}>{item.path}</button>)}</aside><section className="min-w-0"><div className="border-b border-slate-700 px-5 py-3 font-mono text-xs text-slate-300">{file.path} <span className="text-slate-500">@ {file.commit.slice(0, 12)}</span></div><div className="overflow-x-auto py-3 font-mono text-[13px] leading-7">{lines.map(line => { const citation = file.citations.find(item => line.number >= item.start_line && line.number <= item.end_line); return <div key={line.number} className={citation ? 'bg-amber-400/20 text-amber-100' : 'text-slate-300'}><span className="inline-block w-16 select-none border-r border-slate-800 pr-4 text-right text-slate-600">{line.number}</span><span className="pl-5 whitespace-pre">{line.text}</span>{citation?.observation && line.number === citation.start_line && <p className="ml-20 border-l-2 border-amber-400 px-4 text-xs text-amber-200">AI: {citation.observation}</p>}</div> })}</div></section></div>
}
...
function LinkedBugs({ bugs }: { bugs: IssueDetail['bugs'] }) {
  if (!bugs.length) return <p className="text-sm text-slate-400">연결된 버그가 없습니다.</p>
  return <div className="space-y-3">{bugs.map((bug) => <article key={bug.id} className="rounded-xl border border-slate-200 bg-white p-4"><p className="text-sm font-bold text-slate-800">{bug.title}</p><p className="mt-2 font-mono text-[11px] text-slate-400">BUG-{bug.id} · {bug.errorType ?? '오류 유형 없음'}</p>{bug.topStackFrame && <p className="mt-2 rounded bg-slate-50 px-2 py-1 font-mono text-[11px] text-slate-600">{bug.topStackFrame}</p>}<p className="mt-3 text-xs text-slate-400">{formatDate(bug.occurredAt)}</p></article>)}</div>
}

function IssueSummaryMarkdown({ markdown }: { markdown: string | null }) {
  if (!markdown) return <p className="mt-2 text-sm leading-6 text-slate-600">요약이 아직 없습니다.</p>
  ...
  const flushParagraph = () => {
    ...
    blocks.push(<p key={`paragraph-${blocks.length}`} className="mt-3 text-sm leading-6 text-slate-600">{inlineMarkdown(paragraph.join(' '))}</p>)
    paragraph = []
  }
  const flushList = () => {
    if (!list.length) return
    blocks.push(<ul key={`list-${blocks.length}`} className="mt-2 list-disc space-y-1 pl-5 text-sm leading-6 text-slate-600">{list.map((item, index) => <li key={`${item}-${index}`}>{inlineMarkdown(item)}</li>)}</ul>)
    list = []
  }
  ...
  for (const line of lines) {
    ...
    if (heading) {
      flushParagraph()
      flushList()
      blocks.push(<h3 key={`heading-${blocks.length}`} className="mt-5 text-sm font-bold text-slate-800">{inlineMarkdown(heading[1])}</h3>)
    } else if (bullet) {
      flushParagraph()
      list.push(bullet[1])
    } else if (!line.trim()) {
...
function inlineMarkdown(value: string) {
  return value.split(/(\*\*[^*]+\*\*|`[^`]+`)/g).filter(Boolean).map((part, index) => {
    ...
    if (part.startsWith('`') && part.endsWith('`')) return <code key={index} className="rounded bg-slate-100 px-1 py-0.5 font-mono text-[0.85em] text-slate-700">{part.slice(1, -1)}</code>
    return <Fragment key={index}>{part}</Fragment>
  })
}

function AnalysisSection({ analysis, loading, includeEvidence = true }: { analysis: IssueAnalysisSnapshot | null; loading: boolean; includeEvidence?: boolean }) {
  if (loading) return <section className="mt-6"><h3 className="text-xs font-bold text-slate-500">AI 분석</h3><LoaderCircle className="mx-auto mt-5 animate-spin text-clio-600" /></section>
  if (!analysis) return <section className="mt-6"><h3 className="text-xs font-bold text-slate-500">AI 분석</h3><p className="mt-2 text-xs text-slate-400">분석 결과가 없습니다.</p></section>

  const review = analysis.status !== 'COMPLETED'
  const findings = analysis.findings?.map(finding => typeof finding === 'string' ? finding : finding.fact ?? finding.statement).filter(Boolean) ?? []
  ...
  return <section className="mt-6 border-t border-slate-200 pt-5">
    <div className="flex items-center justify-between gap-3"><h3 className="text-xs font-bold text-slate-500">AI 분석</h3><span className={`rounded-full px-2.5 py-1 text-[11px] font-bold ${review ? 'bg-amber-50 text-amber-700' : 'bg-emerald-50 text-emerald-700'}`}>{review ? '검토 필요' : '분석 완료'}</span></div>
    <p className="mt-2 text-xs text-slate-500">신뢰도 {analysis.confidence === undefined ? '—' : `${Math.round(analysis.confidence * 100)}%`}</p>
    {analysis.warnings?.length ? <div className="mt-3 rounded-lg bg-amber-50 p-3 text-xs leading-5 text-amber-800">{analysis.warnings.map(warning => <p key={warning}>{warning}</p>)}</div> : null}
    <AnalysisList title="확인된 원인 및 관찰" values={findings} />
    {hypotheses.length ? <div className="mt-4"><h4 className="text-xs font-bold text-slate-600">원인 가설</h4><div className="mt-2 space-y-2">{hypotheses.map((hypothesis, index) => { const text = typeof hypothesis === 'string' ? hypothesis : hypothesis.hypothesis ?? hypothesis.statement ?? '가설 설명 없음'; const confidence = typeof hypothesis === 'string' ? undefined : hypothesis.confidence; return <div key={`${text}-${index}`} className="rounded-lg border border-slate-200 bg-white p-3"><p className="text-xs leading-5 text-slate-700">{text}</p>{confidence !== undefined && <p className="mt-1 text-[11px] font-bold text-slate-400">신뢰도 {Math.round(confidence * 100)}%</p>}</div> })}</div></div> : null}
    <AnalysisList title="해결 계획" values={plan?.steps?.map(step => typeof step === 'string' ? step : step.description ?? step.title).filter(Boolean) ?? []} />
    <AnalysisList title="완료 기준" values={plan?.acceptance_criteria ?? []} />
    {includeEvidence && <CodeEvidence evidence={analysis.evidence ?? []} />}
  </section>
}

function AnalysisList({ title, values }: { title: string; values: Array<string | undefined> }) {
  const present = values.filter((value): value is string => Boolean(value))
  if (!present.length) return null
  return <div className="mt-4"><h4 className="text-xs font-bold text-slate-600">{title}</h4><ul className="mt-2 space-y-1.5">{present.map((value, index) => <li key={`${value}-${index}`} className="flex gap-2 text-xs leading-5 text-slate-600"><CheckCircle2 size={13} className="mt-0.5 shrink-0 text-clio-600" />{value}</li>)}</ul></div>
}

function CodeEvidence({ evidence }: { evidence: NonNullable<IssueAnalysisSnapshot['evidence']> }) {
  const codeEvidence = evidence.filter(isCodeEvidence)
  if (!codeEvidence.length) return null
  return <div className="mt-4"><h4 className="text-xs font-bold text-slate-600">코드 근거</h4><div className="mt-2 space-y-2">{codeEvidence.map((item, index) => { const location = item.location; const path = item.file_path ?? (typeof location === 'string' ? location : location?.path) ?? '경로 없음'; const start = typeof location === 'string' ? undefined : location?.start_line; const end = typeof location === 'string' ? undefined : location?.end_line; const commit = item.commit ?? item.source_revision; const repositoryId = item.repository_id ?? item.source_id; return <div key={`${path}-${index}`} className="rounded-lg border border-slate-200 bg-white p-3"><p className="break-all font-mono text-[11px] font-bold text-slate-700">{path}{start ? `:${start}${end && end !== start ? `-${end}` : ''}` : ''}</p><p className="mt-1 font-mono text-[10px] text-slate-400">{commit ? commit.slice(0, 12) : 'commit 없음'}{repositoryId ? ` · repository ${repositoryId}` : ''}</p>{item.observation && <p className="mt-2 text-xs leading-5 text-slate-600">{item.observation}</p>}{item.snippet && <pre className="mt-2 overflow-x-auto rounded bg-slate-950 p-2 text-[10px] leading-4 text-slate-100">{item.snippet}</pre>}{item.statement && <p className="mt-2 text-xs leading-5 text-slate-600">{item.statement}</p>}</div> })}</div></div>
}
