...
export function Sidebar({ page, navigate, className, onClose, projects, setProjects, selectedProject, setSelectedProject }) {
  return (
    <aside className={`${className} fixed bottom-0 left-0 top-14 z-30 w-60 flex-col border-r border-slate-200 bg-white`}>
      <div className="flex items-center gap-2 border-b border-slate-100 p-3"><ProjectPicker projects={projects} setProjects={setProjects} selectedProject={selectedProject} setSelectedProject={setSelectedProject} />{onClose && <IconButton className="shrink-0" onClick={onClose} aria-label="사이드바 닫기"><X size={18} /></IconButton>}</div>
